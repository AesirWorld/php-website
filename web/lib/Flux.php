...
class Flux {
	...
	*/
	...
	/**
	* Websites language url
	*/
	public static $languageUrl;
	...
	public static function initialize($options = array())
	{
		...
		self::$language = $langsAvail[$lang];
		self::$languageUrl = $lang;

		// Hack to:
		// Normalize request uri, for the rest of the system to work
		$paths = array_splice($paths, 2);
		$_SERVER['REQUEST_URI'] = '/' . join('/', $paths);
	...
	public static function parseAppConfigFile($filename)
	{
		...
		if (!$themeName = $config->get('ThemeName', false)) {
